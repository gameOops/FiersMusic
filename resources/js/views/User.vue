<template>
    <div>
        <div class="artist_bg" style="display: flex;    align-items: flex-end;">
            <div class="artist_img" style="margin-bottom: 23px;margin-left: 15px;">
                <img id="user_img_page" style='width: 232px;height: 232px;border-radius: 50%;' :src="artist['image']" alt="">
            </div>
            <div class="artist_info" style="    margin-bottom: 12px; background-color: transparent">
                <div class="verify">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M12 21.6596l-3.38079 1.8543-1.84158-3.3877-3.84662-.2679.28231-3.8456-3.09118-2.3049 2.31658-3.0825-1.3543-3.61028 3.61534-1.34071.81255-3.76935 3.76627.82672L12 0l2.7214 2.73168 3.7663-.82672.8125 3.76935 3.6154 1.34071-1.3543 3.61028 2.3166 3.0825-3.0912 2.3049.2823 3.8456-3.8466.2679-1.8416 3.3877L12 21.6596z" fill="#2E77D0"></path><path d="M16.8637 7.41226l-6.6435 7.77824-2.80421-3.2842-.4935.5775 3.29771 3.8617 7.2135-8.44649-.57-.48675z" fill="#fff"></path></svg>
                    <p>Подтвержденный профиль</p>
                </div>
                <div class="artist_name">
                    {{artist['name']}}
                </div>
            </div>

        </div>

    </div>
</template>

<script>

    import axios from 'axios';

    export default {
        name: "Album",

        data() {
            return {
                id:this.$route.params.id,
                name:'',
                albumAll:[],
                artist:[],
                tracks:[]
            }
        },
        methods: {
            loadArtist() {
                let data = {
                    'id': this.id
                };
                axios.post('/api/userId', data)
                    .then(({data}) => {
                        console.log(data);
                        this.artist = data;
                        this.name = data['name']
                        let data2 = {
                            'artist':data['name']
                        };
                        axios.post('/api/selectAll',data2)
                            .then(({data}) => {
                                console.log(data);
                                this.albumAll = data;
                                getColor2();
                            })

                        let data3 = {
                            'name':data['name']
                        };
                        axios.post('/api/selectTracks',data3)
                            .then(({data}) => {
                                console.log(data);
                                this.tracks = data;
                            })
                    })

            },
            showPlay(id) {
                $('#track_'+(id+1)).hide();
                $('#play_'+(id+1)).show();
            },
            hidePlay(id) {
                $('#track_'+(id+1)).show();
                $('#play_'+(id+1)).hide();
            },
            play(src,name,author,img) {
                $('#cur_name').text(name);
                $('#cur_auth').text(author);
                $('#audioplayer').attr('src','/audio/'+src);
                $('#audioplayer').trigger("play");
                $('#play_btn').removeAttr('class');
                $('#play_btn').attr('class','bi bi-pause-circle-fill')
                isPlay = 1;

                $('#cur_img').attr('src',img);
                $('#audio_line').attr('max',document.getElementById('audioplayer').duration);
                $('#audio_line').val(0)

            }
        },
        mounted() {
            this.loadArtist();

        }
    }
</script>
